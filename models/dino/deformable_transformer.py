...
class GatedFusionLayer(nn.Module):
    """
    UGFI Gated Fusion Layer (Gate Fusion Unit).
    A lightweight module that dynamically fuses deep and shallow features
    by learning a pixel-level gating weight via Eq. (7) and Eq. (8) in the paper.
    """
    def __init__(self, d_model):
        super().__init__()
        # Gate unit: takes concatenated features, outputs a gating weight G in (0, 1)
        self.gate_linear = nn.Sequential(
            nn.Linear(d_model * 2, d_model),
            nn.Sigmoid()  # Sigmoid constrains output to (0, 1), initial bias=0 gives G=0.5
        )
        # Linear projections for deep and shallow features (phi_d, phi_s in Eq. 8)
        self.deep_proj = nn.Linear(d_model, d_model)
        self.shallow_proj = nn.Linear(d_model, d_model)
        self.norm = nn.LayerNorm(d_model)

    def forward(self, shallow_feature, deep_feature):
        # Concatenate deep and shallow features to compute gating weight (Eq. 7)
        combined_feature = torch.cat([shallow_feature, deep_feature], dim=-1)

        # Compute gating weight G = sigma(W_g * [F_s^i, F_d])
        gate = self.gate_linear(combined_feature)

        # Project deep and shallow features: phi_d(F_d), phi_s(F_s^i)
        projected_deep = self.deep_proj(deep_feature)
        projected_shallow = self.shallow_proj(shallow_feature)

        # Gated fusion: F_fused = LayerNorm(G * phi_d(F_d) + (1-G) * phi_s(F_s^i))  (Eq. 8)
        fused_feature = gate * projected_deep + (1 - gate) * projected_shallow

        # LayerNorm for training stability
        return self.norm(fused_feature)


# =============================================================================
# Ablation Variants for UGFI Fusion (Table 10, Section 4.4.5)
# =============================================================================

class UGFIAddFusionLayer(nn.Module):
    """
    Naive Skip-Connection via Element-wise Addition (ablation baseline).
    F_fused = LayerNorm(phi_d(F_d) + phi_s(F_s^i))  -- no gating weight.

    Corresponds to "Naive Skip-Connection (Add)" in Table 10.
    Degrades performance because unrestricted shallow injection floods
    the decoder with background clutter (paper: mAP drops to 48.9%).
    """
    def __init__(self, d_model):
        super().__init__()
        self.deep_proj = nn.Linear(d_model, d_model)
    ...
    def forward(self, shallow_feature, deep_feature):
        return self.norm(self.deep_proj(deep_feature) + self.shallow_proj(shallow_feature))


class UGFIConcatFusionLayer(nn.Module):
    """
    Naive Skip-Connection via Concatenation + Projection (ablation baseline).
    F_fused = LayerNorm(W_proj * Concat(F_s^i, F_d))  -- no pixel-level gating.

    Corresponds to "Naive Skip-Connection (Concat)" in Table 10.
    Provides marginal improvement over add (mAP 49.3% vs 48.9%),
    but still inferior to adaptive gated fusion (mAP 49.7%).
    """
    def __init__(self, d_model):
        super().__init__()
    ...
    def forward(self, shallow_feature, deep_feature):
        combined = torch.cat([shallow_feature, deep_feature], dim=-1)
        return self.norm(self.concat_proj(combined))
...
class DeformableTransformer(nn.Module):
    ...
    def __init__(self, d_model=256, nhead=8,
        ...
        # [UGFI Integration]: Initialize UGFI feature fusion layers (Gate Fusion Units).
        # There are (num_encoder_layers - 1) fusion layers because the last layer's features
        # serve as the deep semantic reference (F_d) and are not themselves fused.
        # Each fusion layer follows the reverse hierarchical mapping: i = L - j + 1  (Eq. 6)
        if self.num_encoder_layers > 1:
            self.ufi_fusion_layers = nn.ModuleList([
                GatedFusionLayer(d_model=d_model)
    ...
    def forward(self, srcs, masks, refpoint_embed, pos_embeds, tgt, attn_mask=None):
        ...
        # Begin Encoder
        #########################################################
        # [UGFI Integration]: Modified Encoder returns memory_per_layer: a list of all layer outputs
        memory, enc_intermediate_output, enc_intermediate_refpoints, memory_per_layer = self.encoder(
            src_flatten,
        ...
        # ============================ UGFI Core Logic ============================
        # [UGFI]: U-like Gated Feature Interaction — dynamically fuse deep (last-layer) and
        # shallow encoder features using learned pixel-level gating weights (Eq. 7, Eq. 8).
        # Output list is reversed to match the U-like reverse hierarchy: i = L - j + 1.
        #
        # [Ablation Support — Table 10, Section 4.4.5]:
        # If ablation_config.use_ugfi is False or ugfi_fusion_type is 'none',
        # the UGFI mechanism is bypassed entirely (CHAF-only baseline).
        # Otherwise, the fusion layers (built via build_ugfi_fusion_layer) are used.
        use_ugfi = True
        if hasattr(self, 'ablation_config') and self.ablation_config is not None:
            if not self.ablation_config.use_ugfi or self.ablation_config.ugfi_fusion_type == 'none':
                use_ugfi = False

        if self.num_encoder_layers > 1 and use_ugfi:
            # 1. Extract the deepest encoder layer features as global context (F_d)
            last_layer_memory = memory_per_layer[-1]

            fused_memory_per_layer = []
            # 2. Iterate over all shallow encoder features
            for i in range(self.num_encoder_layers - 1):
                shallow_feature = memory_per_layer[i]  # shallow features F_s^i

                # 3. Apply fusion layer: shallow_feature (F_s^i) + deep_feature (F_d) -> F_fused^i
                # The fusion type is determined by ablation_config.ugfi_fusion_type:
                #   'gated'  -> GatedFusionLayer (Section 3.2, Eq. 7-8)
                #   'add'    -> UGFIAddFusion (naive skip)
                #   'concat' -> UGFIConcatFusion (naive skip)
                fused_layer = self.ufi_fusion_layers[i](
                    shallow_feature=shallow_feature,
                    deep_feature=last_layer_memory
                )
                fused_memory_per_layer.append(fused_layer)

            fused_memory_per_layer.append(last_layer_memory)
            # Reverse to match U-like hierarchy: deep-to-shallow mapping for decoder layers
            ufi_memory_list = fused_memory_per_layer[::-1]
        else:
            # UGFI disabled — pass encoder outputs directly to decoder (reverse order)
            ufi_memory_list = memory_per_layer[::-1]
        # ======================================================================
        ...
        hs, references = self.decoder(
            ...
            # [UGFI Integration]: Pass UGFI-processed feature list into decoder
            memory=ufi_memory_list,
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self,
        ...
        # [UGFI Integration]: Collect full outputs from each encoder layer
        full_layer_outputs = []
        ...
        for layer_id, layer in enumerate(self.layers):
            ...
            # [UGFI Integration]: Append current layer output to the list
            full_layer_outputs.append(output)
        ...
        if self.norm is not None:
            ...
            # [UGFI Integration]: If normalizing the final output, also update the last entry in the list
            full_layer_outputs[-1] = output
        ...
        # [UGFI Integration]: Return the list containing outputs from all encoder layers
        return output, intermediate_output, intermediate_ref, full_layer_outputs
...
class TransformerDecoder(nn.Module):
    ...
    # [UGFI Integration]: Modified forward signature — memory is now a list of Tensors, not a single Tensor
    def forward(self, tgt, memory,
        ...
        Input:
            - tgt: nq, bs, d_model
            - memory: [UGFI] This is a list of num_layers feature maps, each with shape [bs, hw, d_model]
            - pos: hw, bs, d_model
        ...
        for layer_id, layer in enumerate(self.layers):
            ...
            # [UGFI Integration]: Retrieve the encoder feature corresponding to the current decoder layer from the list
            # NOTE: Original memory shape is (hw, bs, d_model), but encoder outputs (bs, hw, d_model), so we transpose
            current_memory_for_layer = memory[layer_id].transpose(0, 1)
            ...
            if not dropflag:
                output = layer(
                    ...
                    # [UGFI Integration]: Pass the current layer's corresponding memory
                    memory=current_memory_for_layer,
...
# ... DeformableTransformerEncoderLayer, DeformableTransformerDecoderLayer, _get_clones, build_deformable_transformer remain unchanged ...
